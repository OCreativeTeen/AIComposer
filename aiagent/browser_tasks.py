...
def ensure_hermes_cdp_chrome(*urls: str, timeout_s: float = 45.0) -> int:
    ...
    Same Chrome instance for ``grv``, ``nbi``, and ``itc``. If CDP is already
    listening (e.g. after ``grv``), reuse it and only open missing URL tabs.
    """
    port = _hermes_cdp_port_live()
    if port:
        log(f"HermesChromeCDP reuse on {port} (grv/nbi/itc share this)")
        if urls:
...
def _grok_attach_cdp_port(*, allow_launch: bool = False) -> int:
    """Attach to HermesChromeCDP. Only ``grv`` may launch Chrome; others reuse existing."""
    port = _hermes_cdp_port_live()
    if port:
        log(f"HermesChromeCDP reuse on {port} (no launch)")
        return port
    if allow_launch:
        return ensure_hermes_cdp_chrome()
    raise RuntimeError(
        "Grok Imagine 还没打开。请先 grv 1 开标签并贴好封面图，再发 gri。"
        f"（HermesChromeCDP 端口 {_grok_cdp_port()} 未连接）"
    )
...
def _notebooklm_generating_via_cdp() -> dict:
    """Read Studio text from the live NotebookLM tab (HermesChromeCDP)."""
    port = _hermes_cdp_port_live()
    if port is None:
        p = _grok_cdp_port()
        return {
            ...
            "count": 0,
            "via": "cdp",
            "error": f"HermesChromeCDP not listening on {p} (grv/nbi/itc share this port)",
        }
    try:
        from playwright.sync_api import sync_playwright
    except Exception as exc:
