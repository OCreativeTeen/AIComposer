...
def _prompt_choice_entries(channel_key: str) -> list[tuple[str, str]]:
    cfg = config.get_channel_config(channel_key) or {}
    entries: list[tuple[str, str]] = []
    for item in cfg.get("scenes_prompt_choices") or []:
        if not item or len(item) < 2:
            continue
        lbl, tpl = item[0], item[1]
        if (tpl or "").strip():
            entries.append(((lbl or "").strip(), tpl))
    return entries


def _resolve_combined_prompt_template(
    channel_key: str,
    *,
    label: str | None = None,
    index: int = 0,
) -> tuple[str, str, int]:
    """在 ``scenes_prompt_choices`` 合并列表中解析 (label, template, index)。"""
    choices = _prompt_choice_entries(channel_key)
    if not choices:
        return ("", "", 0)
    if label:
        want = label.strip()
...
def _scene_entry_heading(entry: dict) -> str:
    """Scene 条目标题：``caption``。"""
    return project_manager.caption_from_scene_content_item(entry)


def _scene_entry_display_text(entry: dict) -> str:
    """单条 scene JSON 条目 → 可读文本（caption + visual）。"""
    if not isinstance(entry, dict):
        return ""
    title = _scene_entry_heading(entry)
    body = (entry.get("visual") or entry.get("content") or entry.get("story") or "").strip()
    if title and body:
        return f"{title}\n\n{body}"
...
def _set_gen_video_clip_segments(video_detail: dict, segments: list[dict]) -> None:
    ...
    for seg in segments or []:
        ...
        src = n["path"]
        try:
            n["path"] = copy_into_gen_video(src)
...
def _is_summary_pdf_file_path(path: str) -> bool:
...
def _gen_video_slide_pdf_dest_filename(video_detail: dict | None) -> str:
    """幻灯片 PDF：与成片 mp4 同 stem，扩展名为 ``.pdf``（``gen_video/<id>.pdf``）。"""
    if isinstance(video_detail, dict):
        for stem in _gen_video_id_stem_candidates_for_row(video_detail):
            if stem:
                return stem + ".pdf"
...
def _flush_accumulated_summary_drop(summary_window: tk.Toplevel) -> None:
    ...
    pdf_in = next(
        (p for p in paths if _is_summary_pdf_file_path(p)),
        None,
    )
    json_in = next(
        (p for p in paths if _is_summary_scene_json_file_path(p)),
        None,
    )
    if mp4_paths:
        ...
        ctx = getattr(summary_window, "_summary_drop_ctx", None)
        if isinstance(ctx, dict):
            _start_summary_slide_pdf_save(pdf_in, summary_window, ctx)
    elif json_in:
        ctx = getattr(summary_window, "_summary_drop_ctx", None)
        if isinstance(ctx, dict):
            _start_summary_scene_content_json_import(
                summary_window, ctx, json_path=json_in
            )
    else:
        try:
            show_auto_close_popup(
...
def _register_summary_gen_media_drop_targets(
    ...
    """注册摘要窗拖放：``.mp4``（审阅裁剪/排序后拼接）加水印成片；图片转 webp 封面；PDF 存为 slide。"""
...
def _on_summary_paste_media_from_clipboard(summary_window: tk.Toplevel) -> None:
    ...
    if image_path:
        ...
        return
    json_path = next(
        (p for p in file_paths if _is_summary_scene_json_file_path(p)),
        None,
    )
    if json_path:
        _start_summary_scene_content_json_import(
            summary_window, ctx, json_path=json_path
        )
        return
    if _try_paste_scene_content_json_from_clipboard(summary_window, ctx):
        return
...
def _on_summary_gen_media_drop(event, summary_window: tk.Toplevel):
    ...
    for p in paths:
        ...
        if low.endswith(".mp4"):
            ...
            image_in = p
        elif _is_summary_pdf_file_path(p) and pdf_in is None:
            pdf_in = p
        elif _is_summary_scene_json_file_path(p) and json_in is None:
            json_in = p
    if mp4_paths:
...
def _refresh_summary_window_title(
    ...
    # 前缀 STORY | 给听筒识别；长 YouTube 标题只留一段，避免截掉前缀。
    if len(title_part) > 72:
...
def _parse_scene_json_list(raw) -> list[dict]:
    """解析 scene_content JSON 文本或 list。"""
...
def _apply_project_video_title(video_detail: dict, title: str) -> None:
    """有 ``project_profile`` 时，用成片名更新 ``video_title``。"""
    if not isinstance(video_detail, dict):
        return
    vt = (title or "").strip()
    if not vt:
...
def _apply_titles_to_scene_content_first_entry(video_detail: dict, title: str) -> bool:
    """若已有 scene_content，将首条 ``caption`` 同步为给定标题。"""
    vt = (title or "").strip()
    if not vt or not isinstance(video_detail, dict):
        return False
    sc = video_detail.get("scene_content")
    if not isinstance(sc, list) or not sc or not isinstance(sc[0], dict):
        return False
    first = copy.deepcopy(sc[0])
    project_manager.normalize_scene_content_item_for_workflow(first)
    first["caption"] = vt
    first.pop("title", None)
    first.pop("Title", None)
    sc = list(sc)
    sc[0] = first
    video_detail["scene_content"] = sc
...
def _scene_captions_for_cover_title(video_detail: dict) -> list[str]:
    ...
    for item in sc:
        if not isinstance(item, dict):
            continue
        cap = project_manager.caption_from_scene_content_item(item)
        if cap:
            out.append(cap)
    return out


def _cover_save_story_title(video_detail: dict) -> str:
    """当前故事名：成片名优先，否则列表行原标题。"""
    return (
        _youtube_row_display_title(video_detail)
        or _youtube_row_source_title(video_detail)
    ).strip()


def _cover_title_preset_options(
    video_detail: dict,
    *,
    image_path: str = "",
) -> list[tuple[str, str]]:
    """``(下拉显示, 填入成片名)``；故事原名始终第一项。"""
    ...
    def add(label: str, value: str) -> None:
        ...
        if not v or v in seen:
            return
        seen.add(v)
        shown = v if len(v) <= 56 else v[:56] + "…"
        options.append((f"{label}：{shown}", v))

    add("故事原名", _cover_save_story_title(video_detail))
    img = _title_from_cover_image_path(image_path)
    if img:
        add("图片文件名", img)
    for i, cap in enumerate(_scene_captions_for_cover_title(video_detail), 1):
        add(f"场景 {i}", cap)
    if not options:
        add("故事原名", "")
    return options
...
def _cover_save_default_video_title(
    ...
    *,
    ...
    """封面前对话框默认：保持故事原名。"""
    _ = image_path
    return _cover_save_story_title(video_detail)
...
def _apply_video_title_before_cover_save(
    ...
    *,
    ...
    """保存封面前：仅更新 ``project_profile.video_title``（不改场景 caption / 原视频标题）。"""
    if not isinstance(video_detail, dict):
        return
    vt = (video_title or "").strip()
    if not vt:
        return
    _apply_project_video_title(video_detail, vt)
...
def _ask_video_title_before_cover_save_dialog(
    ...
    *,
    ...
    """保存封面前让用户确认成片名；默认保持故事原名。取消则放弃本次粘贴/拖放。"""
    if not isinstance(video_detail, dict):
        return None
    ...
    story_title = _cover_save_story_title(video_detail)
    presets = _cover_title_preset_options(video_detail, image_path=image_path)
    label_to_value = {lab: val for lab, val in presets}
    result_holder: dict | None = None
    dlg = tk.Toplevel(parent)
    dlg.title("保存封面前 — 确认视频标题")
    dlg.geometry("720x280")
    dlg.minsize(520, 240)
    ...
    dlg.geometry(f"720x280+{(sw - 720) // 2}+{(sh - 280) // 2}")

    frm = ttk.Frame(dlg, padding=12)
    frm.pack(fill=tk.BOTH, expand=True)

    hint = (
        "默认保持故事原名。可用下拉改用图片文件名或某个场景的 caption，"
        "也可以在下方输入框里自己改。"
        "不会修改原视频标题（YouTube 下载名），也不会改各场景 caption。"
    )
    if has_project_profile:
        hint += " 若选用其它名字，会更新 project_profile.video_title。"
    ...
    pick_box = ttk.LabelFrame(frm, text="选用名称", padding=8)
    pick_box.pack(fill=tk.X, pady=(0, 8))
    combo_var = tk.StringVar(value=presets[0][0])
    combo = ttk.Combobox(
        pick_box,
        textvariable=combo_var,
        values=[lab for lab, _ in presets],
        state="readonly",
        width=86,
    )
    combo.pack(fill=tk.X)

    title_box = ttk.LabelFrame(frm, text="成片名（可手改）", padding=8)
    title_box.pack(fill=tk.X, pady=(0, 10))
    title_var = tk.StringVar(value=story_title)
    ...
    def on_preset_selected(_event=None) -> None:
        lab = (combo_var.get() or "").strip()
        val = label_to_value.get(lab, "")
        title_var.set(val)
        title_entry.selection_range(0, tk.END)
    ...
    def on_confirm():
        ...
        skip = (not vt) or (vt == story_title)
        result_holder = {"video_title": vt, "skip_title_update": skip}
        dlg.destroy()
...
def save_cover_image_as_gen_video_webp(
    ...
    *,
    ...
    if out_ok:
        if isinstance(video_detail, dict):
            video_detail["cover_image"] = out_ok
        return True, out_ok, ""
...
def accept_summary_cover_image(
  ...
  * ``interactive=True``（拖放）：弹窗确认成片名；默认保持故事原名。
  * ``interactive=False``（``itc N`` / Telegram）：不弹窗、不改故事名。
    """
    mgr = ctx.get("mgr")
    vd = ctx.get("vd")
    if mgr is None or not isinstance(vd, dict):
...
def _start_summary_slide_pdf_save(
    ...
    *,
    ...
    """PDF 幻灯片：复制到 gen_video 并写入 video_detail['slide']。"""
    mgr = ctx.get("mgr")
    vd = ctx.get("vd")
    if mgr is None or not isinstance(vd, dict):
        return
    ...
    gen_dir = getattr(config, "INPUT_MEDIA_GEN_VIDEO_PATH", "") or ""
    dest_name = _gen_video_slide_pdf_dest_filename(vd)
    ...
    def _worker():
        ...
        try:
            os.makedirs(gen_dir, exist_ok=True)
            dest_abs = os.path.join(gen_dir, dest_name)
            safe_copy_overwrite(pdf_path, dest_abs)
            out_ok = os.path.abspath(dest_abs)
        except Exception as ex:
            err_msg = str(ex)
        finally:
            ...
            if out_ok:
                ...
                if _persist_channel_videos(mgr):
                    ...
                    show_auto_close_popup(
                        par,
                        "幻灯片已保存",
                        f"PDF 已复制到 gen_video 并记录为 slide：\n{out_ok}",
                    )
                else:
                    show_auto_close_popup(
                        par, "保存失败", "PDF 已复制但写入频道列表失败。", kind="error"
                    )
            elif err_msg:
...
def _run_summary_gen_video_clip_review(
    ...
    *,
    ...
    """打开审阅窗 → 保存片段到 scene_content clip 字段 → 拼接加水印写入 gen_video['video']。"""
...
def _downloader_aux_dir(youtube_dir: str) -> str:
    """下载器辅助文件目录（cookies、频道 info 等）。

    频道 GUI（``youtube_dir=<channel>/Download``）→ ``program/<channel>``（原 ``Download/work``）。
    项目内 ``MediaDownloader`` → 项目目录 ``youtube_dir`` 本身。
    """
    norm = os.path.normpath(youtube_dir or "")
    if os.path.basename(norm) == "Download":
...
def _youtube_row_scene_meta_title(video_detail: dict) -> str:
    """scene_content 首条 caption。"""
    if not isinstance(video_detail, dict):
        return ""
    sc = video_detail.get("scene_content")
...
def _youtube_row_list_suffix_title(video_detail: dict) -> str:
    """列表行标题后缀：``project_profile.video_title`` > scene 首场景 caption。"""
    if not isinstance(video_detail, dict):
        return ""
    proj = _youtube_row_project_title(video_detail)
    if proj and proj not in _GENERIC_PROJECT_VIDEO_TITLES:
        return proj
    scene_meta = _youtube_row_scene_meta_title(video_detail)
    if scene_meta:
        return scene_meta
    sc = video_detail.get("scene_content")
...
def _sync_project_video_title_on_list_row(
    ...
    """``project_profile.video_title`` 优先 scene 首场景 caption，避免「新项目」占位。"""
...
def _video_detail_source_keys(vd: dict) -> tuple[str, str]:
    """源 YouTube 行的 ``id`` / ``url``，用于匹配 ``cloned_from_*``。"""
    if not isinstance(vd, dict):
        return "", ""
    return (str(vd.get("id") or "").strip(), str(vd.get("url") or "").strip())


def _find_cloned_project_pids_for_source(vd: dict, channel_videos: list) -> list[str]:
    """同源克隆项目 pid 列表（主题分表里「新建项目」追加的行，``cloned_from_id/url`` 指向源行）。"""
    src_id, src_url = _video_detail_source_keys(vd)
    if not src_id and not src_url:
        return []
    out: list[str] = []
    seen: set[str] = set()
    for item in channel_videos or []:
        if not isinstance(item, dict):
            continue
        pid = _video_detail_project_pid(item)
        if not pid or pid in seen:
            continue
        cf_id = (item.get("cloned_from_id") or "").strip()
        cf_url = (item.get("cloned_from_url") or "").strip()
        matched = (src_id and cf_id == src_id) or (src_url and cf_url and cf_url == src_url)
        if matched:
            seen.add(pid)
            out.append(pid)
    return out
...
def _linked_project_pids_for_video_detail(vd: dict, channel_videos: list | None) -> list[str]:
    """本条已绑 pid，或从同源克隆行解析出的全部 pid（保持列表顺序）。"""
    direct = _video_detail_project_pid(vd)
    if direct:
        return [direct]
    return _find_cloned_project_pids_for_source(vd, channel_videos or [])


def _list_row_index_for_pid(channel_videos: list, pid: str) -> int:
    pid = (pid or "").strip()
    if not pid:
        return -1
    for i, item in enumerate(channel_videos or []):
        if _list_json_item_matches_pid(item, pid):
            return i
    return -1
...
def _find_gen_video_mp4_for_row(video: dict) -> str:
    if isinstance(video, dict):
        stored = (video.get("video") or "").strip()
...
def _find_gen_video_webp_for_row(video: dict) -> str:
    """封面：优先故事 ``cover_image``，否则 ``gen_video/<id>.webp``。"""
    if isinstance(video, dict):
        stored = (video.get("cover_image") or "").strip()
        if stored and os.path.isfile(stored):
            return os.path.abspath(stored)
    gen_dir = getattr(config, "INPUT_MEDIA_GEN_VIDEO_PATH", "") or ""
    if not gen_dir:
...
def _find_gen_video_slide_for_row(video: dict) -> str:
    """``gen_video/<id>.pdf`` 或 video_detail['slide'] 记录的幻灯片路径。"""
    if not isinstance(video, dict):
        return ""
    stored = (video.get(GEN_VIDEO_SLIDE_KEY) or "").strip()
    if stored and os.path.isfile(stored):
        return os.path.abspath(stored)
    gen_dir = getattr(config, "INPUT_MEDIA_GEN_VIDEO_PATH", "") or ""
    if not gen_dir:
        return ""
    for stem in _gen_video_id_stem_candidates_for_row(video):
        p = os.path.join(gen_dir, stem + ".pdf")
        if os.path.isfile(p):
            return os.path.abspath(p)
    return ""
...
def _copy_file_to_clipboard_hdrop(parent, file_path: str) -> bool:
    """将文件复制到 Windows 剪贴板（CF_HDROP），便于粘贴到资源管理器或其它应用。"""
    import struct

    file_path = (file_path or "").strip()
    if not file_path or not os.path.isfile(file_path):
        messagebox.showwarning("提示", "文件不存在。", parent=parent)
        return False
    abs_path = os.path.normpath(os.path.abspath(file_path))
    try:
        import win32clipboard  # type: ignore

        file_list = abs_path + "\0\0"
        file_list_bytes = file_list.encode("utf-16-le")
        dropfiles_offset = 20
        dropfiles = struct.pack("Iiiii", dropfiles_offset, 0, 0, 0, 1)
...
def _collect_feature_media_clipboard_options(
    video_detail: dict,
) -> list[tuple[str, str, str]]:
    """汇总可复制的成片媒体：(option_id, 按钮文案, 文件路径)。"""
    if not isinstance(video_detail, dict):
        return []
    ...
    if webp_p:
        options.append(
            ("webp", f"封面图片 · {os.path.basename(webp_p)}", webp_p)
        )
    slide_p = _find_gen_video_slide_for_row(video_detail)
    if slide_p:
        options.append(
            ("slide", f"幻灯片 PDF · {os.path.basename(slide_p)}", slide_p)
        )
    mp4_p = _find_gen_video_mp4_for_row(video_detail)
    if mp4_p:
        options.append(
            ("mp4", f"成片视频 · {os.path.basename(mp4_p)}", mp4_p)
        )
    return options


def _copy_feature_media_to_clipboard(parent, video_detail: dict) -> bool:
    """复制封面/幻灯片/成片到剪贴板；多项时弹窗让用户选择。"""
    options = _collect_feature_media_clipboard_options(video_detail)
    if not options:
        messagebox.showwarning(
            "提示",
            "尚无可复制的媒体。\n请拖入图片（封面）、PDF（幻灯片）或 MP4（成片）到本窗口。",
            parent=parent,
        )
        return False

    if len(options) == 1:
        option_id, picked_label, file_path = options[0]
    else:
        picked = askchoice(
            "封面复制",
            [(oid, lbl) for oid, lbl, _ in options],
            parent=parent,
        )
        if not picked:
            return False
        picked_id = picked[1] if isinstance(picked, (tuple, list)) else picked
        match = next((o for o in options if o[0] == picked_id), None)
        if not match:
            return False
        option_id, picked_label, file_path = match

    if option_id == "webp":
        ok = _copy_image_file_to_clipboard(parent, file_path)
    else:
        ok = _copy_file_to_clipboard_hdrop(parent, file_path)
    if ok:
        show_auto_close_popup(
            parent,
            "已复制",
...
def _scene_content_nonempty_value(scene_content) -> bool:
    """``scene_content`` 是否为非空 list 或可解析的非空 JSON。"""
    if not scene_content:
        return False
    if isinstance(scene_content, list):
        return bool(scene_content)
    if isinstance(scene_content, str):
        t = scene_content.strip()
        if not t:
            return False
        try:
            parsed = json.loads(t)
...
def _video_detail_has_scene_content(vd: dict) -> bool:
    if not isinstance(vd, dict):
        return False
    return _scene_content_nonempty_value(vd.get("scene_content"))


def _video_detail_can_create_project(vd: dict) -> bool:
    """有 ``scene_content`` 即可新建项目；``analyzed_content`` 可选。"""
    return _video_detail_has_scene_content(vd)


def _scene_content_list_from_parsed(parsed) -> list | None:
    if isinstance(parsed, list) and parsed:
        items = [copy.deepcopy(x) for x in parsed if isinstance(x, dict)]
...
def _normalize_scene_content_list(scenes: list) -> list:
    out: list = []
    for item in scenes:
        if not isinstance(item, dict):
            continue
        cp = copy.deepcopy(item)
        project_manager.normalize_scene_content_item_for_workflow(cp)
        out.append(cp)
    return out


def _read_scene_content_from_json_path(path: str) -> list | None:
    path = (path or "").strip()
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            raw = f.read()
        parsed = config.parse_json_from_text(raw)
        return _scene_content_list_from_parsed(parsed)
    except OSError:
        return None


def _apply_scene_content_import_to_video_detail(
    video_detail: dict,
    scenes: list,
    mgr,
    *,
    refresh_tree_fn=None,
    refresh_title_fn=None,
) -> int:
    normalized = _normalize_scene_content_list(scenes)
    if not normalized or not isinstance(video_detail, dict):
        return 0
    video_detail["scene_content"] = normalized
    ch_path = getattr(mgr, "channel_path", "") or ""
    _normalize_channel_list_item_for_storage(video_detail, ch_path)
    _persist_channel_videos(mgr)
    if callable(refresh_tree_fn):
        try:
            refresh_tree_fn()
        except Exception:
            pass
    if callable(refresh_title_fn):
        try:
            refresh_title_fn()
        except Exception:
            pass
...
def _start_summary_scene_content_json_import(
    summary_window: tk.Toplevel,
    ctx: dict,
    *,
    json_path: str = "",
    ...
    scenes = None
    if json_path:
        scenes = _read_scene_content_from_json_path(json_path)
    elif json_text:
        parsed = config.parse_json_from_text(json_text)
        scenes = _scene_content_list_from_parsed(parsed)
    if not scenes:
        try:
            show_auto_close_popup(
                summary_window,
                "场景 JSON",
                "无法解析为场景 JSON array（每项须为 object，含 speaking/caption 等字段）。",
                kind="error",
            )
        except Exception:
            pass
    ...
    n = _apply_scene_content_import_to_video_detail(
        vd,
        scenes,
        mgr,
        refresh_tree_fn=ctx.get("refresh_channel_tree"),
        refresh_title_fn=ctx.get("refresh_title_fields"),
    )
    rfn_feat = ctx.get("refresh_feature_media_row")
    if callable(rfn_feat):
        try:
    ...
    show_auto_close_popup(
        summary_window,
        "场景 JSON 已导入",
        f"已写入 scene_content：{n} 个场景。",
    )
    return True


def _try_paste_scene_content_json_from_clipboard(
    summary_window: tk.Toplevel, ctx: dict
) -> bool:
    text = _read_host_clipboard_text(summary_window).strip()
    if not text:
        return False
    head = text.lstrip()
    ...
    if not scenes:
        return False
    return _start_summary_scene_content_json_import(
        summary_window, ctx, json_text=text
    )


def _is_summary_scene_json_file_path(path: str) -> bool:
    return (path or "").lower().endswith(".json")


def _fill_prompt_picker_scroll_section(
    ...
    *,
    ...
    if not enabled:
        ttk.Label(
            ...
            justify=tk.CENTER,
            wraplength=360,
        ).pack(expand=True, pady=48, padx=12)
        return
    ...
    if not choices:
        ttk.Label(inner, text="（未配置提示词）", foreground="#888888").pack(
            anchor=tk.W, padx=8, pady=12
        )
        return

    for lbl, tpl in choices:
        def _make_cmd(label=lbl, template=tpl):
            def _cmd():
                on_pick(label, template)
        ...
        ttk.Button(
            ...
            text=lbl,
            command=_make_cmd(),
        ).pack(fill=tk.X, padx=4, pady=3, ipady=4)


def show_image_slide_prompt_picker_dialog(
    parent,
    *,
    slide_enabled: bool,
    image_enabled: bool,
    ...
    dlg.geometry(f"960x620+{(sw - 960) // 2}+{(sh - 620) // 2}")

    outer = ttk.Frame(dlg, padding=12)
    outer.pack(fill=tk.BOTH, expand=True)
    ttk.Label(
        outer,
        text="选择提示词模板，点击即复制到剪贴板（含媒体路径与项目信息）。",
        wraplength=900,
    ...
    slide_title = "Slide · PDF 幻灯片分析"
    if slide_path:
        slide_title += f"\n（{os.path.basename(slide_path)}）"
    slide_frame = ttk.LabelFrame(panes, text=slide_title, padding=8)
    panes.add(slide_frame, weight=1)

    image_title = "Image · 封面单图视频"
    if webp_path:
        image_title += f"\n（{os.path.basename(webp_path)}）"
    image_frame = ttk.LabelFrame(panes, text=image_title, padding=8)
    panes.add(image_frame, weight=1)

    _fill_prompt_picker_scroll_section(
        slide_frame,
        config_prompt.SLIDE_ANALYSIS_PROMPT_CHOICES,
        enabled=slide_enabled,
        disabled_hint="尚无幻灯片 PDF。\n请先拖入 PDF 到摘要窗。",
        on_pick=lambda lbl, tpl: (on_slide_pick(lbl, tpl), dlg.destroy()),
    )
    _fill_prompt_picker_scroll_section(
        image_frame,
        config_prompt.DIRECT_VIDEO_PROMPT_CHOICES,
        enabled=image_enabled,
        disabled_hint="尚无封面图片。\n请先拖入图片或 Ctrl+V 粘贴封面。",
        on_pick=lambda lbl, tpl: (on_image_pick(lbl, tpl), dlg.destroy()),
    )
...
class MediaDownloader:
    ...
    def _find_cookies_file(self):
        ...
        if os.path.exists(download_cookies):
            ...
            # 移动到项目路径
            project_cookies = os.path.join(_downloader_aux_dir(self.youtube_dir), cookies_filename)
    ...
    def list_hot_videos(self, channel_url, max_videos=5000, min_view_count=500):
        ...
        try:
            ...
            info_path = os.path.join(_downloader_aux_dir(self.youtube_dir), f"info_{channel_name}.json")
...
class MediaGUIManager:
    ...
    def _combined_prompt_text_for_label(
        self,
        video_detail: dict,
        prompt_label: str,
        *,
        ...
        lbl, template, idx = _resolve_combined_prompt_template(
    ...
    def _generate_scene_content_from_notebooklm_prompt(
        ...
        *,
        ...
        if isinstance(parsed, list):
            out: list[dict] = []
            for it in parsed:
                n = _normalize_scene_dict_entry(it) if isinstance(it, dict) else None
                if n:
                    out.append(n)
    ...
    def _run_scene_smart_generate_async(
        ...
        *,
        ...
        if not (video_detail.get("analyzed_content") or "").strip():
            ...
            return False
        if not prompt_label and not _prompt_choice_entries(channel_key):
            messagebox.showwarning(
                "提示",
                "当前频道未配置可用的 LM 提示词，无法智能生成。",
                parent=parent,
            )
            return False
        if on_busy:
        ...
        def work():
            ...
            def apply():
                ...
                new_title = project_manager.caption_from_scene_content_item(scenes[0])
                if new_title:
                    _apply_project_video_title(video_detail, new_title)
    ...
    def _copy_notebooklm_scene_instruction(
        self,
        *,
        parent,
        video_detail: dict,
        ...
        if clip_body and (channel_path or "").strip():
            try:
                base, var = config_prompt.normalize_nb_export_mode(nb_mode, nb_variant)
                export_key = f"{base}/{var}"
            except ValueError:
                export_key = nb_mode
            channel_clipboard_append_item(channel_path, clip_body, export_key)
        if status_msg:
            try:
    ...
    def _copy_image_prompt_instruction(
        ...
        *,
        ...
        entries = _parse_scene_json_list(story_raw)
    ...
    def _copy_slide_prompt_instruction(
        self,
        *,
        parent,
        video_detail: dict,
        ...
        """选择 Slide/PDF 分析指令模板并复制到剪贴板。"""
        if not (template or "").strip():
            choices = config_prompt.SLIDE_ANALYSIS_PROMPT_CHOICES
            if not choices:
                show_auto_close_popup(
                    parent, "Slide Prompt", "未配置 slide 分析提示词。", kind="error"
                )
                return False

            pick_labels = [lbl for lbl, _ in choices]
            picked = askchoice("Slide Prompt", pick_labels, parent=parent)
            if not picked:
                return False
            picked_label = picked[1] if isinstance(picked, (tuple, list)) else picked
        ...
        slide_path = _find_gen_video_slide_for_row(video_detail)
        clip_body = config_prompt.build_slide_analysis_clipbody(
            instruction=template,
        ...
        if clip_body and (channel_path or "").strip():
            channel_clipboard_append_item(
                channel_path, clip_body, "slide_analysis_instruction"
            )
        hint = f"已拷贝到剪贴板：{picked_label}"
        if not slide_path:
            hint += "\n\n（提示：尚未拖入 PDF；可将 PDF 拖入本窗后 slide 路径会自动附带。）"
        show_auto_close_popup(parent, "Slide Prompt", hint)
        return bool(clip_body)

    def _open_image_slide_prompt_picker(
        self,
        parent,
        video_detail: dict,
        *,
        ...
        """打开双栏提示词选择窗（Slide / Image）。"""
        webp_p = _find_gen_video_webp_for_row(video_detail)
        slide_p = _find_gen_video_slide_for_row(video_detail)
        has_webp = bool(webp_p)
        has_slide = bool(slide_p)
        if not has_webp and not has_slide:
            messagebox.showwarning(
                "提示",
                "尚无封面图片或幻灯片 PDF。\n请先拖入图片或 PDF 到本窗口。",
                parent=parent,
        ...
        show_image_slide_prompt_picker_dialog(
            parent,
            slide_enabled=has_slide,
            image_enabled=has_webp,
            slide_path=slide_p,
            webp_path=webp_p,
            on_slide_pick=lambda lbl, tpl: self._copy_slide_prompt_instruction(
                parent=parent,
                video_detail=video_detail,
            ...
            on_image_pick=lambda lbl, tpl: self._copy_image_prompt_instruction(
                parent=parent,
                video_detail=video_detail,
                story_raw=story_raw,
        ...
        *,
        ...
        def _build_editor_ui():
            ...
            def _get_content_bridge() -> str:
                ...
                if txw is None:
                    return ""
                return (txw.get("1.0", tk.END) or "").strip()

            def _set_content_bridge(value: str):
                txw = scene_ui.get("tx")
                if txw is None:
                    return False, "scene_content 编辑区仍在加载，请几秒后重试"
                try:
                    txw.delete("1.0", tk.END)
                    if value:
                        txw.insert("1.0", value)
                except tk.TclError as exc:
                    return False, f"scene_content set failed: {exc}"
                if value:
                    ok, _msg, parsed = apply_scene_content_from_editor_text(
                        video_detail, value
                    )
            ...
            def _persist_content_bridge():
                txw = scene_ui.get("tx")
                if txw is None:
                    return False, "scene_content 编辑区仍在加载，请几秒后重试"
                raw = (txw.get("1.0", tk.END) or "").strip()
            ...
            def _scene_list_for_bridge() -> list | None:
                txw = scene_ui.get("tx")
                if txw is not None:
                    raw = (txw.get("1.0", tk.END) or "").strip()
                    if raw:
            ...
            def _set_scene_choice_early(value: str):
                ...
                if low in ("all", "全部", "0", "-1"):
                    ...
                    return False, (
                        ...
                        + "（用 all / 1 / 2 / …）\nchoices: "
                        + " | ".join(_scene_choice_labels_bridge())
                    )
                btn = scene_ui.get("scene_index_btn")
                if btn is not None:
                    try:
                        btn.config(
                            text=_scene_index_button_label_bridge(_scene_copy_index[0])
                        )
                    except tk.TclError:
                        pass
                scene_msg = (
                    f"scene_choice={_scene_index_button_label_bridge(_scene_copy_index[0])}"
                )
                if _scene_copy_index[0] >= 0:
                    ...
                    copied = self._copy_notebooklm_scene_instruction(
                        parent=dlg,
                        video_detail=video_detail,
                        scenes=_scene_list_for_bridge() or [],
                        nb_mode=base,
                        nb_variant=var,
                        main_character=main_character,
            ...
            def _nb_choice_rows_bridge():
                lang = config.llm_language_label(getattr(self, "language", "") or "")
                return config_prompt.notebooklm_export_flat_choices(lang)

            def _nb_labels_bridge():
                return [row[0] for row in _nb_choice_rows_bridge()]

            def _set_notebooklm_early(value: str):
                rows = _nb_choice_rows_bridge()
            ...
            bind_screen(
                ...
                dlg,
                {
                    ...
                    "content": {
                        ...
                        "set": _set_content_bridge,
                        "persist": _persist_content_bridge,
                    },
                    "scene_choice": {
                        "get": lambda: _scene_index_button_label_bridge(
                            _scene_copy_index[0]
                        ),
        ...
        def _build_editor_ui_rest():
            ...
            def _fill_scene_editor_rest():
                ...
                def _insert_scene_json_chunked(scene_json: str, pos: int = 0) -> None:
                    """Small slices so Tk can pump the CLI bridge between inserts."""
                    if not scene_json:
                        return
                    end = min(pos + _SCENE_JSON_CHUNK, len(scene_json))
                    try:
                        if pos == 0:
                            tx.insert("1.0", scene_json[pos:end])
                        else:
                            tx.insert(tk.END, scene_json[pos:end])
                    except tk.TclError:
                        return
                    if end < len(scene_json):
                        dlg.after(
                            5,
                            lambda e=end, s=scene_json: _insert_scene_json_chunked(s, e),
                        )

                def _sync_tx_from_video_detail() -> None:
                    """Show video_detail.scene_content in editor (no validation)."""
                    try:
                        if (tx.get("1.0", tk.END) or "").strip():
                            return
                    except tk.TclError:
                        return
                    scenes = video_detail.get("scene_content") or []
                    ...
                    def _worker() -> None:
                        ...
                        try:
                            dlg.after(
                                0,
                                lambda s=scene_json: _insert_scene_json_chunked(s, 0),
                            )
                        except tk.TclError:
                            pass
                    ...
                    threading.Thread(
                        target=_worker, daemon=True, name="scene-json-dump"
                    ).start()

                scene_ui["_sync_tx_from_video_detail"] = _sync_tx_from_video_detail
                dlg.after_idle(_sync_tx_from_video_detail)

                def _fill_scene_editor_rest_body():
    ...
    def _format_analyze_prompt(self, video_detail=None) -> str:
        raw = config.get_channel_analyze_prompt(
    ...
    def _bind_yt_text_download_channel_list(self) -> None:
        ...
        ch_id = config.get_channel_id(self.channel)
    ...
    def _show_channel_videos_dialog(self, *, auto_open_summary_row_keys: list[str] | None = None):
        ...
        def smart_select():
            ...
            for item in tree.get_children():
                ...
                analyzed = video_detail.get("analyzed_content") or ""
        ...
        def _run_project_start_for_video_detail(
            ...
            *,
            ...
            channel_videos = self.downloader.channel_videos or []
            existing_pid = (bound_pid or _video_detail_project_pid(vd) or "").strip()
            if not existing_pid:
                linked = _find_cloned_project_pids_for_source(vd, channel_videos)
                if len(linked) == 1:
                    existing_pid = linked[0]
            list_idx = -1
            try:
                list_idx = channel_videos.index(vd)
            except ValueError:
                pass
            ...
            def _load_bound_project_cfg(for_pid: str = ""):
                wanted = (for_pid or existing_pid or "").strip()
                cfg = None
                if wanted:
                    row_ix = _list_row_index_for_pid(channel_videos, wanted)
                    if row_ix >= 0 and list_path and os.path.isfile(list_path):
                        item = channel_videos[row_ix]
                        cfg = project_manager.project_config_from_list_item(
                            item, list_path, row_ix
                        )
                if (not cfg or not cfg.get("pid")) and wanted:
                    cfg = project_manager.load_project_config_by_pid(wanted)
                return cfg
            ...
            def _create_new_project_from_raw(
                ...
                *,
                ...
                if not _scene_content_nonempty(
                    ...
                    return
                ac = (analyzed_content or "").strip() if analyzed_content else ""
            ...
            if forced_action == "open":
                if not existing_pid:
                    ...
                    return
                cfg = _load_bound_project_cfg(existing_pid)
        ...
        def on_focus(event, low_priority=False):
            ...
            if summary_window_ref.get("w") and summary_window_ref["w"].winfo_exists():
                ...
                try:
                    ...
                    set_screen_ready(config.SCREEN_STORY_ROOT, False)
                except Exception:
                    pass
            ...
            def _cli_open_clip_review(value: str):
                ...
                if segments:
                    ...
                    preview = "\n".join(
                        f"  场景 {i}. {os.path.basename(seg['path'])}"
                    ...
                    preview = "\n".join(
                        f"  场景 {i}. {os.path.basename(p)}"
                        for i, p in enumerate(paths, 1)
                    )
                    n = len(paths)
                return (
                    True,
                    f"已打开审阅窗（{n} 段，按 scene_content 场景 1→{n} 顺序）。"
                    f"请在窗口内调整并确认生成成片。\n{preview}",
                )
            ...
            def _mark_story_bridge_ready():
                try:
                    from gui.cli_bridge import set_screen_ready

                    set_screen_ready(config.SCREEN_STORY_ROOT, True)
                except Exception:
                    pass

            summary_window.after_idle(_mark_story_bridge_ready)
        ...
        def export_selected_choices():
            ...
            messagebox.showinfo(
                ...
                + f"\n\n{out_path}\n\n"
                "Telegram / Hermes 取用：打开 cli\\run_bot.bat（听筒会从队列打开摘要）。\n"
                "纯手工：不必开听筒，在本列表里打开摘要即可。",
                parent=dialog,
            )
